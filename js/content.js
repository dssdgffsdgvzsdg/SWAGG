...
// Same idea as fetchTierMinimum, but for the exponential score curve's
// boundary specifically: it needs the worst (highest) rank among ALL
// levels at diffDivider or harder — not just levels at exactly
// diffDivider. Normally these give the same answer, since the worst
// Mythical-tier level naturally sits at that boundary. But if the
// Mythical tier is currently empty (no levels with difficulty ===
// diffDivider at all), fetchTierMinimum would return 0 and break the
// curve — this keeps working correctly by falling through to whatever
// tier IS currently the "easiest" one in the exponential band (e.g.
// Silent, if that's all that's been added so far).
export function fetchExpBoundary(list, diffDivider) {
    let boundary = 0;
    list.forEach(([err, rank, level]) => {
        if (err) {
            return;
        ...
        if (level.difficulty >= diffDivider) {
            boundary = Math.max(rank, boundary);
        }
    });
